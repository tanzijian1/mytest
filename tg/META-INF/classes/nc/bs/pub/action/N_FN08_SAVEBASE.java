...
public class N_FN08_SAVEBASE extends AbstractPfAction<AggCostAccrueBill> {

	@Override
	protected CompareAroundProcesser<AggCostAccrueBill> getCompareAroundProcesserWithRules(
			Object userObj) {
		CompareAroundProcesser<AggCostAccrueBill> processor = null;
		AggCostAccrueBill[] clientFullVOs = (AggCostAccrueBill[]) this.getVos();
		if (!StringUtil.isEmptyWithTrim(clientFullVOs[0].getParentVO()
				.getPrimaryKey())) {
			processor = new CompareAroundProcesser<AggCostAccrueBill>(
					CostAccrueBillPluginPoint.SCRIPT_UPDATE);
		} else {
			processor = new CompareAroundProcesser<AggCostAccrueBill>(
					CostAccrueBillPluginPoint.SCRIPT_INSERT);
		}
		// TODO 在此处添加前后规则
