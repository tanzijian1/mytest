 ...
 * 
 */
public class ContTranBillUtils extends BillTranUtils {
	static ContTranBillUtils utils;

	public static ContTranBillUtils getUtils() {
		if (utils == null) {
			utils = new ContTranBillUtils();
		}
		return utils;
	}
	...
	protected void setHeaderVO(PayableBillVO hvo, PayBillHeaderVO headvo)
		...
		// 紧急程度->紧急程度
		hvo.setAttributeValue("def10", "通用EBS系统");// 自定义项10 来源外部系统
		...
		// 根据code 获取地区财务审批状态2020-02-25-谈子健
		if (StringUtils.isNotBlank(headvo.getAuditstate())) {
			String pk_defdoc = getAuditstateByCode(headvo.getAuditstate());
			if (pk_defdoc == null) {
	...
	protected void checkItemNotNull(PayBillItemVO bodyvo)
		...
		/*
		 * if (StringUtils.isBlank(bodyvo.getPaymenttype())) { throw new
		 * BusinessException("款项类型不可为空"); }
		 */
	...
	protected void checkHeaderNotNull(PayBillHeaderVO headvo)
		...
		/*
		 * if (StringUtils.isBlank(headvo.getTotalmny_request())) { throw new
		 * BusinessException("累计请款金额不可为空"); } if
		 * (StringUtils.isBlank(headvo.getTotalmny_pay())) { throw new
		 * BusinessException("累计付款金额不可为空"); } if
		 * (StringUtils.isBlank(headvo.getIsshotgun())) { throw new
		 * BusinessException("是否先付款后补票不可为空"); } if
		 * (StringUtils.isBlank(headvo.getTotalmny_paybythisreq())) { throw new
		 * BusinessException("本次请款累计付款金额不可为空"); } if
		 * (StringUtils.isBlank(headvo.getTotalmny_inv())) { throw new
		 * BusinessException("累计发票金额不可为空"); }
		 * 
		...
		// if (StringUtils.isBlank(headvo.getPleasetype())) {
