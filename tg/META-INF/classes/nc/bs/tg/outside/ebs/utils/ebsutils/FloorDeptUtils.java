 ...
 *
 */
public class FloorDeptUtils extends EBSBillUtils{

	static FloorDeptUtils utils;

	public static FloorDeptUtils getUtils() {
		if (utils == null) {
			utils = new FloorDeptUtils();
		}
		return utils;
	}
...
//							"isnull(dr,0)=0 and pk_defdoclist = '"+DefdocUtils.getUtils().getDefdoclistByCode("lcbm")+"'");
			
			List<Map> docVOList = DefdocUtils.getUtils().getBdDefdoc("lcbm",(String) value.get("starttime"),(String) value.get("endtime"));
//			if (docVO != null&&docVO.size()>0) {
//				for (DefdocVO defdocVO : docVO) {
//					Map<String, Object> postdata = new HashMap<String, Object>();// 整体报文
//					postdata.put("pk_defdoc",defdocVO.getPk_defdoc());//自定义档案pk
