...
public class MoreToolsUtil extends SaleBillUtils{
	static MoreToolsUtil utils;
	Boolean extraBill = false;//是否补票
	public static MoreToolsUtil getUtils() {
		if (utils == null) {
			utils = new MoreToolsUtil();
		}
		return utils;
	}
public AggOutputTaxHVO[] moreOutputinvoice(String jsonstr,int actiontype) throws BusinessException{	
	InvocationInfoProxy.getInstance().setGroupId("000112100000000005FD");
	InvocationInfoProxy.getInstance().setUserDataSource(DataSourceName);
   InvocationInfoProxy.getInstance().setUserId(getSaleUserID());
	AggOutputTaxHVO[] aggvos = null;
	HashMap<String, AggOutputTaxHVO[]> map=new HashMap<String, AggOutputTaxHVO[]>();
	...
	if(aggvos==null&&existaggVO!=null){//nc系统存在开票单据，直接返回开票单据
		existaggVO.getParentVO().setSellMachineNo(aggoJsonvo.getSerialNo());
		 return new AggOutputTaxHVO[]{existaggVO};
	}
...
public void checkrule(NcOutputtaxJsonVO vo,int actiontype) throws BusinessException{
	...
	AggOutputTaxHVO existaggVO=null;
	/**
	 * 外系统调用工具，单据保存
	 * 
	 * @param json
	...
	private OutputTaxHVO buildNcheadVO(NcOutputtaxJsonVO optjsonvo)
		...
		String invoicer=optjsonvo.getInvoicer();
		headvo.setDef17(invoicer);// 开票人
