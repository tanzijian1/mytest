package nc.bs.tg.outside.salebpm.servlet;
...
public class SaleBPMBillStatesServlet extends HttpServlet implements
		IHttpServletAdaptor {
	...
	public void doAction(HttpServletRequest request,
		...
		byte[] sc = SyncSaleBPMBillStatesUtils.getUtils().token(
				"ISyncSaleBPMBillServcie:" + josn, "onSyncBillState_RequiresNew");
		BPMBillStateResultVO resultVO = new BPMBillStateResultVO();
		InvocationInfoProxy.getInstance().setUserDataSource(
				SyncSaleBPMBillStatesUtils.DataSourceName);// 设置全局数据源
		...
		try {
			...
			logVO.setDesbill(ISaleBPMBillCont.getBillNameMap()
					.get(value.getBilltypeName()));
			logVO.setPrimaryKey(value.getBpmid());
			logVO.setOperator(value.getOperator() == null
					|| "".equals(value.getOperator()) ? SyncSaleBPMBillStatesUtils.BPMOperatorName
					: value.getOperator());
			String result = NCLocator.getInstance()
					.lookup(ISyncSaleBPMBillServcie.class)
					.onSyncBillState_RequiresNew(value,josn);
			resultVO.setRsmstate(SyncSaleBPMBillStatesUtils.STATUS_SUCCESS);
			resultVO.setMsg(result);
			logVO.setResult(SyncSaleBPMBillStatesUtils.STATUS_SUCCESS);
			logVO.setErrmsg(result);
		} catch (Exception e) {
			Logger.error(e.getMessage(), e);
			resultVO.setRsmstate(SyncSaleBPMBillStatesUtils.STATUS_FAILED);
			...
			logVO.setErrmsg(org.apache.commons.lang.exception.ExceptionUtils
					.getFullStackTrace(e));		} finally {
			SyncSaleBPMBillStatesUtils.getUtils().restore(sc);
			try {
				SyncSaleBPMBillStatesUtils.getUtils().getBaseDAO().insertVO(logVO);
