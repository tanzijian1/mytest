...
public class CostAdvanceBillUtil extends BillUtils implements ITGSyncService {
	...
	public String onSyncInfo(HashMap<String, Object> info, String methodname)
		...
		String billqueue = "费用预提" + ":" + srcid;
		String billkey = "费用预提" + ":" + srcno;
		String destype = "01";
		...
		AggAccruedBillVO aggVO = (AggAccruedBillVO) getBillVO(
				AggAccruedBillVO.class, "isnull(dr,0)=0 and defitem1 = '"
						+ srcid + "'");
		if (aggVO != null
				&& (headVO.getNcid() == null || headVO.getNcid() == "")
				&& !("Y".equals(headVO.getIsapprove()))) {
		...
		BillUtils.addBillQueue(billqueue);// 增加队列处理
		try {
			// 删除单据
			if ("Y".equals(headVO.getIsdelete())
					&& !(StringUtils.isEmpty(headVO.getNcid()))) {
				...
				if ("Y".equals(headVO.getIsdelete())) {
					AggAccruedBillVO deleaggvo = (AggAccruedBillVO) getBillVO(
							...
							"isnull(dr,0)=0 and  pk_accrued_bill = '"
									+ headVO.getNcid() + "'");
					if (deleaggvo == null) {
					...
					if (deleaggvo.getParentVO().getBillstatus() == 3) {
						Object returnobj_uapp = pfaction.processAction(
								IPFActionName.UNAPPROVE, "262X", null,
								deleaggvo, null, null);
					...
					if (deleaggvo.getParentVO().getApprstatus() != -1) {
						returnobj = pfaction.processAction(
								IPFActionName.UNSAVE, "262X", null, deleaggvo,
								null, null);
					}
					if (returnobj != null) {
					...
					getAppService().deleteVOs(
							new AggAccruedBillVO[] { deleaggvo });
					aggvo = deleaggvo;
				}
			} else {
				if (!(StringUtils.isEmpty(headVO.getNcid()))
						&& !("Y".equals(headVO.getIsapprove()))) {// 更新单据
					AggAccruedBillVO prebillvo = new AggAccruedBillVO();
					...
					if (preupdateAggvo.getParentVO().getApprstatus() == 1) {//
						...
						prebillvo = iser
								.ytunapprove_RequiresNew(preupdateAggvo);
					} else {
						prebillvo = preupdateAggvo;
					}
					operation.append("更新操作");
					updedbillvo = updateVO(headVO, prebillvo, bodyVOs);
					try {
						// updedbillvo.getParentVO().setHasntbcheck(new
						// UFBoolean(flag));
						...
						// }
						updedbillvo.getParentVO()
								.setHasntbcheck(UFBoolean.TRUE);
						aggvo = iser.ytupdate_RequiresNew(updedbillvo);
					}
					...
					AggAccruedBillVO billvo = onTranBill(headVO, bodyVOs);
					AccruedDetailVO[] bvoss = billvo.getChildrenVO();
					AggAccruedBillVO nbillvo = (AggAccruedBillVO) billvo
							.clone();
					...
					try {
						...
						AggAccruedBillVO ybillvo = onTranBill(headVO, bodyVOs);
	...
	public AggAccruedBillVO updateVO(CostHeadVO headvo, AggAccruedBillVO aggvo,
			List<CostBodyVO> bodyVOs) throws Exception {
		...
		// update_hvo.setAttributeValue("pk_group",
		// getpk_groupByCode(headvo.getOrg()));// 所属集团
		...
		// update_hvo.setAttributeValue("pk_tradetype", "262X-Cxx-FY01");//
		// 交易类型编号
		// } else {
		...
		update_hvo.setAttributeValue("defitem2", headvo.getSrcbillcode());// EBS单号
		...
		// update_hvo.setAttributeValue("defitem27", headvo.getPlatedetail());//
		// 商业板块明细
		...
		// update_hvo.setAttributeValue("creationtime", new UFDateTime());//
		// 创建时间
		update_hvo.setAttributeValue("modifier", null);// 最后修改人
		update_hvo.setAttributeValue("modifiedtime", null);// 最后修改时间
		update_hvo.setAttributeValue("auditman", getUserIDByCode(""));// 审批流发起人
		...
		int row = 1;
		String assume_org=update_hvo.getPk_org();
		...
		for (CostBodyVO costBodyVO : bodyVOs) {
			...
			save_bodyVO
					.setAttributeValue("pk_project", costBodyVO.getProject());// 项目getpk_projectByCode()
			...
			// save_bodyVO.setAttributeValue("defitem2",
			...
			save_bodyVO
					.setAttributeValue("pk_project", costBodyVO.getProject());
	...
	public AggAccruedBillVO onTranBill(CostHeadVO headvo,
		...
		save_headVO.setAttributeValue("operator_org",
				getPk_orgByCode(headvo.getOperatororg()));// 经办人单位
		save_headVO.setAttributeValue("operator_dept",
				getOperator_dept(headvo.getOperatordept(), save_headVO.getOperator_org()));// 经办人部门
		save_headVO.setAttributeValue("operator",
				getopertor(headvo.getOperator()));// 经办人
		// save_headVO.setAttributeValue("operator_org",
		...
		// save_headVO.setAttributeValue("defitem27",
		// headvo.getPlatedetail());// 商业板块明细
		...
		// getUserPkByCode(headvo.getCreator()));// 审批流发起人
		save_headVO.setAttributeValue("auditman",
				getUserIDByCode(headvo.getCreator()));
		...
		List<AccruedDetailVO> save_bodyVOs = new ArrayList<>();
		String assume_org = save_headVO.getPk_org();
		String assume_dept = getOperator_dept("12", assume_org);
		int row = 1;
		for (CostBodyVO costBodyVO : bodyVOs) {
			AccruedDetailVO save_bodyVO = new AccruedDetailVO();
	...
	private void checkHeadTransforExpense(CostHeadVO headvo)
			throws BusinessException {
		if (StringUtils.isBlank(headvo.getOrg())) {
			throw new BusinessException("出账公司不可为空");
		}
		if (StringUtils.isBlank(headvo.getSrcid())) {
			throw new BusinessException("外部来源主键不可为空");
		}
		if (StringUtils.isBlank(headvo.getSrcbillcode())) {
			throw new BusinessException("外部来源单号不可为空");
		}
		if (StringUtils.isBlank(Float.valueOf(headvo.getAmount()).toString())) {
			throw new BusinessException("金额不可为空");
		}
		// if (StringUtils.isBlank(headvo.getOperatororg())) {throw new
		// BusinessException("经办人单位不可为空");}
		// if (StringUtils.isBlank(headvo.getOperator())) {throw new
		// BusinessException("经办人不可为空");}
		// if (StringUtils.isBlank(headvo.getOperatordept())) {throw new
		// BusinessException("经办人部门不可为空");}
		// if (StringUtils.isBlank(headvo.getReason())) {throw new
		// BusinessException("事由不可为空");}
		// if (StringUtils.isBlank(headvo.getCreator())) {throw new
		// BusinessException("制单人不可为空");}
	}

	private void checkBodyTransforExpense(CostBodyVO bodyvo)
			throws BusinessException {
		if (StringUtils.isBlank(bodyvo.getAssumeorg())) {
			throw new BusinessException("费用承担单位不可为空");
		}
		// if (StringUtils.isBlank(bodyvo.getAssumedept())) {throw new
		// BusinessException("费用承担部门不可为空");}
