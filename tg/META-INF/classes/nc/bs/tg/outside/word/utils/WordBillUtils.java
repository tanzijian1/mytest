...
public class WordBillUtils {
	static WordBillUtils utils;
	public static final String DataSourceName = "design";// 接口默认数据源
	public static final String OperatorName = "WORD";// BPM默认操作员
	...
	public static WordBillUtils getUtils() {
		if (utils == null) {
			utils = new WordBillUtils();
		}
		return utils;
	}
	 ...
	 * @throws BusinessException
	 */
	public List<String> getYearMthList(UFDate begin, UFDate end)
			throws BusinessException {
		String sql = "select distinct t.yearmth from bd_accperiodmonth t where t.dr = 0 and t.begindate is not null and t.yearmth between ? and ?  "
				+ " order by yearmth";
		SQLParameter parameter = new SQLParameter();
		parameter.addParam(begin.getYear() + "-" + begin.getStrMonth());
		parameter.addParam(end.getYear() + "-" + end.getStrMonth());
		List<String> list = (List<String>) getBaseDAO().executeQuery(sql,
				parameter, new ColumnListProcessor());
		return list;
	}
