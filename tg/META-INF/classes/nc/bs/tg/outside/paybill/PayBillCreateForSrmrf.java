...
public class PayBillCreateForSrmrf extends PayBillUtils {
	...
	protected void setHeaderVO(PayBillVO hvo, JSONObject head)
		...
		hvo.setAttributeValue(PayBillVO.PK_ORG, orgInfo.get("pk_org"));// 应付财务组织->NC业务单元编码
		...
		hvo.setAttributeValue("def13", recaccountInfo.get("bankname"));// 开户行名称/自定义项28
		hvo.setAttributeValue("def14", recaccountInfo.get("bankcode"));// 开户行编码/自定义项25
		...
		hvo.setAttributeValue("def22", psnInfo.get("pk_psndoc"));// 申请人
	...
	protected void setItemVO(PayBillVO headVO, PayBillItemVO itemVO,
		...
		itemVO.setAttributeValue(PayBillItemVO.SUPPLIER, headVO.getSupplier());// 供应商
		itemVO.setAttributeValue(PayBillItemVO.SCOMMENT, headVO.getScomment());// 摘要
		
		itemVO.setAttributeValue(PayBillItemVO.RECACCOUNT, headVO.getSupplier());//收款账户
		...
		if (StringUtils.isNotBlank(invoicetype)) {
			...
			if (invTypeInfo == null) {
				throw new BusinessException("发票类型[" + budgetsub
						+ "]未能在NC档案关联到相关信息!");
			}
		...
		itemVO.setAttributeValue(PayBillItemVO.TAXRATE, UFDouble.ZERO_DBL);// 税率
		itemVO.setAttributeValue(PayBillItemVO.LOCAL_NOTAX_DE, local_money_cr);// 不含税金额
		itemVO.setAttributeValue(PayBillItemVO.MONEY_DE, local_money_cr);// 借方原币金额
																			// //
		// 价税合计->付款计划明细金额
		itemVO.setAttributeValue(PayBillItemVO.LOCAL_MONEY_DE,
				itemVO.getMoney_de());// 本币金额
		itemVO.setAttributeValue(PayBillItemVO.LOCAL_TAX_DE, UFDouble.ZERO_DBL);// 税额->付款计划明细不含税金额*税率
		...
		itemVO.setAttributeValue(PayBillItemVO.DEF8, headVO.getDef19());// *请款事由/事由
