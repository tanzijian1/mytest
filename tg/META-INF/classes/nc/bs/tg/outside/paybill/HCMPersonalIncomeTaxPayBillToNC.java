...
public class HCMPersonalIncomeTaxPayBillToNC extends PayBillUtils implements
	...
	protected AggPayBillVO onTranBill(PersonalIncomeTaxHeadVO headVO,
		...
		save_headVO.setAttributeValue("pk_balatype", getBalatypePkByCode(headVO.getPk_balatype()));// 结算方式
		save_headVO.setAttributeValue("billdate", headVO.getBilldate());// 扣款日期
		save_headVO.setAttributeValue("def67", headVO.getDef67().substring(0,7));// 工资所属月
		...
		save_headVO.setAttributeValue("billmaker",getUserIDByCode(HCMSalaryOperatorName));// 制单人
		save_headVO.setAttributeValue("objtype", 1); // 往来对象
														// 2=部门，3=业务员，1=供应商，0=客户
		save_headVO.setAttributeValue("creator", getUserIDByCode(HCMSalaryOperatorName));// 创建人
		...
		for (PersonalIncomeTaxBodyVO personalIncomeTaxBodyVO : bodyVOs) {
			...
			save_bodyVO.setAttributeValue("money_de",
					personalIncomeTaxBodyVO.getMoney_de());// 实发合计 原币
			save_bodyVO.setAttributeValue("local_money_bal",
					personalIncomeTaxBodyVO.getMoney_de());// 本币=原币（币种一样）
			save_bodyVO.setAttributeValue("local_money_de",
					personalIncomeTaxBodyVO.getMoney_de());// 余额
			save_bodyVO.setAttributeValue("pausetransact", UFBoolean.FALSE);// 挂起标志
			save_bodyVO
			.setAttributeValue("pk_balatype", save_headVO.getPk_balatype());// 结算方式
			save_bodyVO
			...
			save_bodyVO.setAttributeValue("objtype", save_headVO.getObjtype());// 往来对象
																				// 2=部门，3=业务员，1=供应商，0=客户
