...
/**
 * 调用公积金付款单生成公积金往来应付单 单据转换规则
 * 
 * @author zhaozhiying
 * 
 */
public class ApproveF3FundPayBillAfterDataListener extends EBSBillUtils
	...
	public void doAction(IBusinessEvent event) throws BusinessException {
		...
		try {
			...
			for (AggregatedValueObject vo : aggPayBillVOs) {
				...
				PayBillItemVO[] itemVos = (PayBillItemVO[]) billVO
						.getAllChildrenVO();
	 ...
	 */
	public void convertBill(PayBillVO headVo, PayBillItemVO bodyvo)
			throws BusinessException {
		...
		try {
			...
			payableBillVO.setPk_tradetype("F1-Cxx-015");
			payableBillVO.setPk_tradetypeid("1001341000000057B89E");
			...
			payableBillVO.setSupplier(bodyvo.getDef18());
			 ...
			 */
			itemVO.setDef1(bodyvo.getDef1());
			itemVO.setDef36(bodyvo.getDef36());
			...
			itemVO.setAttributeValue(PayableBillItemVO.DIRECTION,
					    BillEnumCollection.Direction.CREDIT.VALUE);// 方向s
...
*/
			...
			//aggVo.getParentVO().setAttributeValue(PayableBillVO.MONEY, money);
			aggVo.getParentVO().setAttributeValue(PayableBillVO.LOCAL_MONEY,
					bodyvo.getMoney_de());
			aggVo.getParentVO().setAttributeValue(PayableBillVO.GROUPLOCAL,
					bodyvo.getMoney_de());
			aggVo.getParentVO().setAttributeValue(PayableBillVO.GLOBALLOCAL,
					bodyvo.getMoney_de());
			...
			Object obj = (AggPayableBillVO[]) getPfBusiAction().processAction(
					"SAVE", "F1", null, aggVo, null, eParam);
			...
			obj = (AggPayableBillVO[]) getPfBusiAction().processAction("SAVE",
					"F1", worknoteVO, billvos[0], null, eParam);
	 ...
	 * @throws BusinessException
	 */
	public void changeBillByRule(PayBillVO headVo, PayBillItemVO bodyvo)
			throws BusinessException {
		...
		try {
			AggregatedValueObject newbusivo = exchangeservice().runChangeData(
					"F3-Cxx-031", "F1-Cxx-015", billVO, null);
			...
			if (istrue) {
				...
				eParam.put(PfUtilBaseTools.PARAM_NOTE_CHECKED,
						PfUtilBaseTools.PARAM_NOTE_CHECKED);
				getPfBusiAction().processAction("SAVE", "F1", null, aggVo,
						null, eParam);
			}
		} catch (BusinessException e) {
			throw new BusinessException(e.getMessage(), e);
	...
	public String getSupplierIDByPkOrg(String pk_org) throws BusinessException {
		...
		String sql = "select c.pk_supplier from bd_supplier c where c.pk_financeorg = (select org.pk_financeorg from org_financeorg org where org.pk_financeorg ='"
				+ pk_org + "')";
		// String result = (String) getBaseDAO().executeQuery(sql,new
		// ColumnProcessor());
		String result = (String) service.executeQuery(sql,
				new ColumnProcessor());
		return result;
	}
