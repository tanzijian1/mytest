package nc.ui.tg.financingtotal.ace.panel;
...
import nc.vo.tg.financingtotal.FinancingTotal;
...
public class ImportablePanel extends BillImportablePanel {
	...
	protected AggregatedValueObject getTransData(
		...
		AggFinancingTotal aggVO = new AggFinancingTotal();
		FinancingTotal fcp = new FinancingTotal();
		...
		for (BillItem item : items) {
			if (getFilterFiled().contains(item.getKey())) {
				resultList
					...
					|| item.getKey().contains("moon_total")
