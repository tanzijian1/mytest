	...
	public void handleAppEvent(AddEvent e) {
		...
		if(userpk!=null){
			try {
				...
				panel.getHeadItem("def24").setValue(up_psnpk);//跟进人上级
				panel.getHeadItem("def23").setValue(pk_psndoc);//跟进人
		...
		panel.getHeadItem("def13").setEnabled(false);//应付账到期日
		panel.getHeadItem("def14").setEnabled(false);//专项计划到期日
		panel.getHeadItem("def20").setEnabled(false);//循环购买金额
	}
}
